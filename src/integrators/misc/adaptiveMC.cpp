...
/*!\plugin{adaptiveMC}{Adaptive Monte Carlo integrator}
 * \order{13}
 * \parameters{
 ...
 * }
 */
class AdaptiveIntegratorMC : public MonteCarloIntegrator {
public:
	AdaptiveIntegratorMC(const Properties &props) : MonteCarloIntegrator(props) {
	...
	AdaptiveIntegratorMC(Stream *stream, InstanceManager *manager)
...
MTS_IMPLEMENT_CLASS_S(AdaptiveIntegratorMC, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(AdaptiveIntegratorMC, "Adaptive integrator for MonteCarloIntegrators");
MTS_NAMESPACE_END
